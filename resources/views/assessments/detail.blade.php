...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    @php
        ...
        $badgeIcon = match($result->conclusion->severity) {
            'normal' => 'M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z',
            'ringan' => 'M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z',
            'sedang' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4.5c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z',
            'tinggi' => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4.5c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z',
            default => 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4.5c-.77-.833-2.694-.833-3.464 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z',
        };
    ...
    <div class="relative mt-10 mb-4">
        ...
        <div class="relative overflow-hidden rounded-[2rem] shadow-2xl" style="background: linear-gradient(145deg, {{ $concColor }} 0%, {{ $concColor }}dd 60%, {{ $concColor }}bb 100%)">
            ...
            <div class="absolute inset-0 opacity-[0.06]">
                ...
                <div class="absolute top-1/2 right-1/4 w-4 h-4 rounded-full bg-white"></div>
            </div>
            ...
            <div class="relative px-6 sm:px-8 py-8 sm:py-10">
                ...
                <div class="flex items-center justify-between mb-8">
                    <div class="flex items-center gap-3">
                        <div class="w-12 h-12 rounded-2xl bg-white/15 backdrop-blur flex items-center justify-center">
                            <svg class="w-7 h-7 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="{{ $badgeIcon }}"/></svg>
                        </div>
                        <div>
                            <p class="text-xs font-semibold uppercase tracking-[0.15em] text-white/50">Kesimpulan</p>
                            <h2 class="text-xl sm:text-2xl font-extrabold text-white leading-tight">{{ $result->conclusion->title }}</h2>
                        </div>
                    </div>
                    <span class="inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wider bg-white/15 backdrop-blur text-white border border-white/10">
                        <span class="w-2 h-2 rounded-full bg-white shadow-[0_0_6px_rgba(255,255,255,0.6)]"></span>
                        {{ $sevLabel }}
                    </span>
                </div>
                ...
                <div class="relative rounded-3xl p-6 sm:p-7 bg-white/10 backdrop-blur-md border border-white/10 shadow-inner">
                    ...
                    <p class="text-white/95 text-sm sm:text-base font-medium leading-relaxed mt-2">
                ...
                <div class="mt-8 text-center">
                    <a href="{{ $result->conclusion->reference_link }}" target="_blank" rel="noopener" class="group inline-flex items-center gap-4 px-7 py-4 rounded-2xl bg-white text-sm font-bold transition-all duration-300 hover:scale-[1.03] hover:shadow-2xl active:scale-[0.98] shadow-lg shadow-black/10" style="color: {{ $concColor }}">
                        <svg class="w-5 h-5 group-hover:animate-pulse flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9"/></svg>
                        <span>Lihat Rekomendasi Penanganan</span>
                        <svg class="w-4 h-4 group-hover:translate-x-1.5 transition-transform duration-300 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/></svg>
                    </a>
                </div>
                @endif
            </div>
